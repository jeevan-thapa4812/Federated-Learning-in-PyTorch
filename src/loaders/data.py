...
from src import stratified_split, TqdmToLogger
...
def load_dataset(args):
    ...
    if client_datasets is None:
        ...
        if (args.eval_type == 'local') and (args.test_size == -1):
            ...
            holdout_sets = [SubsetWrapper(holdout_set, f'< {str(idx).zfill(8)} > (test)') for idx, holdout_set in
                            enumerate(holdout_sets)]
            augmented_datasets = []
