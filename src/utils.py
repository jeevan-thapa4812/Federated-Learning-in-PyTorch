...
class TensorboardServer(Process):
    ...
    def run(self):
        if self.os_name == 'nt':  # Windows
            os.system(
                f'{sys.executable} -m tensorboard.main --logdir "{self.path}" --host {self.host} --reuse_port=true --port {self.port} 2> NUL')
        elif self.os_name == 'posix':  # Linux
            os.system(
                f'{sys.executable} -m tensorboard.main --logdir "{self.path}" --host {self.host} --reuse_port=true --port {self.port} >/dev/null 2>&1')
        else:
...
def check_args(args):
    ...
    if 'cuda' in args.device:
        ...
        # check optimizer
    if not((args.optimizer in torch.optim.__dict__.keys()) or (args.optimizer in ['DAdaptation', 'DAdaptationSplit', 'DAdaptSGD'])):
